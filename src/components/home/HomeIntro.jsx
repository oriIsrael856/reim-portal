...
const HomeIntro = ({ data }) => {
    return (
        <div className="relative w-full max-w-[1200px] mx-auto px-4 -mt-10 z-20 mb-24">
            {/* אייקון מטוס בנייר (כתום במובייל) */}
            <div className="absolute -top-8 left-1/2 transform -translate-x-1/2 z-20">
                <div className="bg-[#FFB84C] md:bg-[#FFD028] p-3 rounded-2xl rotate-12 shadow-[4px_4px_0px_rgba(0,0,0,0.1)] border-2 border-white">
                    <Send className="text-white transform rotate-45" size={28} fill="white" />
                </div>
            </div>

            {/* הבוקסה הסגולה */}
            <div className="bg-[#5E3BEE] text-white rounded-[40px] px-6 py-12 md:px-32 md:py-20 text-right md:text-center shadow-2xl relative overflow-hidden">
                <div className="relative z-10">
                    <h3 className="text-purple-200 text-sm font-bold mb-3 tracking-widest uppercase">{data.subtitle}</h3>
                    <h2 className="text-3xl md:text-4xl md:text-5xl font-black mb-6">{data.title}</h2>
                    <p className="text-base md:text-lg md:text-xl leading-relaxed opacity-90 max-w-4xl md:mx-auto font-light whitespace-pre-line">
                        {data.text}
                    </p>
                </div>
